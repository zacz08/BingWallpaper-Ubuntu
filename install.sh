...
echo "Triggers: ${HOUR}:$(printf '%02d' "$MINUTE"), 0:00, 19:00, plus on login/wake."
echo "Trigger now:  launchctl kickstart -k gui/$UID_NUM/$LABEL"
echo
echo "OPTIONAL — make the Mac auto-wake before the daily trigger so it"
echo "fires even when the lid is closed (runs in dark wake, ~10s):"
WAKE_HOUR="$HOUR"
WAKE_MIN="$MINUTE"
# Wake 5 min before the scheduled run.
if [ "$WAKE_MIN" -ge 5 ]; then
    WAKE_MIN=$((WAKE_MIN - 5))
else
    WAKE_MIN=$((WAKE_MIN + 55))
    WAKE_HOUR=$(((WAKE_HOUR + 23) % 24))
fi
